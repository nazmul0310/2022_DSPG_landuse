...
```{r}
plot(income[,"median"])
ggplot(income, aes(fill = median))+ geom_sf()+ scale_fill_distiller(palette="Reds", direction=1)+ labs(title="Median Income by Census Tract", caption="ACS5 2016-2020", fill="Income") 
```
```{r}
...
ggplot(new.age.df, aes(x=name , y=value, fill=new.age.df$name))+geom_bar(stat="identity") + coord_flip() + scale_color_brewer(palette = "Set3") + theme(legend.position="none") + labs(title="Proportion of Age Groups", y= "Proportion of Population", x= "Age Groups", caption="ACS5 2016-2020")
