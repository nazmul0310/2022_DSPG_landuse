...
county = "Goochland"
year = 2020
fileName <- paste("/Review/Christopher/ACS_rObjects/ACS", year, county, ".rData", sep="")
```

```{r importing data}
setwd("C:/Users/Christopher Vest/Documents/Land_use_project/2022_DSPG_Landuse")
...
```{r median income}
plot(income)
income$variable <- factor(income$variable, levels = income$variable)
ggplot(income, aes(x = variable, y = estimate, fill = variable))+ geom_bar(stat = "identity") + theme(legend.position = "none") + scale_fill_viridis(discrete=TRUE) + labs(title="Median Income by Proportion of population", x = "Proportion of population", y = "Median income", caption="Source: ACS5 2016-2020") + coord_flip()
```
